...
class Aircraft extends CI_Controller {
    ...
    function index($index = 'pre_owned', $manufacture_type = null, $manufacture_name = null) {
        ...
        $this->db->where('sub_category_description', 'jets');
        $this->db->or_where('sub_category_description', 'twin_turbo_prop');
        $this->db->or_where('sub_category_description', 'turbo_prop');
        $this->db->or_where('sub_category_description', 'twin_piston');
        ...
        $this->page_data->secondary_title  = str_replace('_', '-', ucwords($this->page_data->filter)) .' '. ucwords(__class__) . ' For Sale';
        $this->page_data->page_title        = str_replace('_', '-', ucwords($this->page_data->filter)). ' Aircraft For Sale';
    ...
    function details($name = '', $flight_id) {
        ...
        $this->page_data->secondary_title  = str_replace('_', '-', ucwords($this->page_data->flight->sales_type_description)) .' '. ucwords(__class__) . ' For Sale';
